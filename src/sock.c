...
void sock_connect(xmpp_sock_t *xsock)
{
    ...
    do {
        ...
        if (conn->sock != INVALID_SOCKET) {
            ...
            if (rc == 0)
                rc = sock_set_nonblocking(conn->sock);
            if (rc == 0)
                rc = connect(conn->sock, ainfo->ai_addr, ainfo->ai_addrlen);
        ...
        strophe_debug(xsock->ctx, "sock", "sock_connect() result %d",
                      conn->sock);
